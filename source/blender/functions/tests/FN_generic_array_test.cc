...
TEST(generic_array, MoveConstructor)
{
  ...
  EXPECT_EQ(array_b.type(), array_a.type());
  EXPECT_EQ(array_b.size(), 10);
  EXPECT_EQ(typed_span_b[4], 42);

  /* Make sure the copy constructor cleaned up the original array. */
  EXPECT_TRUE(array_a.data() == nullptr);
  EXPECT_EQ(array_a.size(), 0);
  EXPECT_TRUE(array_a.is_empty());
  EXPECT_TRUE(&array_a.type() != nullptr); /* But it shouldn't clear the type. */
}
