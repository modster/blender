...
vec2 tile_to_ndc(vec2 tile_to_uv_fac, vec2 tile_co, vec2 offset)
{
  /* Add a margin to prevent culling too much if the frustum becomes too much unstable. */
  tile_co += /* cull_data.tile_margin * */ offset;
  return tile_co * tile_to_uv_fac * 2.0 - 1.0;
}

CullingTile culling_tile_get(vec2 tile_to_uv_fac, uvec2 tile_co)
{
  vec2 ftile = vec2(tile_co);
  /* Culling frustum corners for this tile. */
  vec3 corners[8];
  corners[0].xy = corners[4].xy = tile_to_ndc(tile_to_uv_fac, ftile, vec2(1, 1));
  corners[1].xy = corners[5].xy = tile_to_ndc(tile_to_uv_fac, ftile, vec2(1, 0));
  corners[2].xy = corners[6].xy = tile_to_ndc(tile_to_uv_fac, ftile, vec2(0, 0));
  corners[3].xy = corners[7].xy = tile_to_ndc(tile_to_uv_fac, ftile, vec2(0, 1));
