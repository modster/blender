...
void planes_setup(Pyramid shape, out vec4 planes[5])
{
  ...
  vec3 S1 = shape.corners[2] - shape.corners[1];
  vec3 S2 = shape.corners[2] - shape.corners[3];

  planes[0] = plane_setup(shape.corners[0], A1, A2);
  planes[1] = plane_setup(shape.corners[0], A2, A3);
  planes[2] = plane_setup(shape.corners[0], A3, A4);
  planes[3] = plane_setup(shape.corners[0], A4, A1);
  planes[4] = plane_setup(shape.corners[1], S1, S2);
}
