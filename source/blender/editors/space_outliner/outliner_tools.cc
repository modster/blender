                                          ...
                                          void *user_data)
{
  ...
  if (ID_IS_OVERRIDE_LIBRARY_REAL(id_root)) {
    ...
    BlendFileReadReport file_read_report = {};
    file_read_report.reports = reports;
    ...
    ReportList *reports, Main *bmain, ViewLayer *view_layer, Scene *scene, Base *base)
{
  ...
  object = base->object;
  for (child_base = (Base *)view_layer->object_bases.first; child_base; child_base = base_next) {
...
using OutlinerDeleteFunc = void (*)(bContext *, ReportList *, Scene *, Object *);
...
static int outliner_action_set_exec(bContext *C, wmOperator *op)
{
  ...
  /* get action to use */
  act = (bAction *)BLI_findlink(&bmain->actions, RNA_enum_get(op->ptr, "action"));
