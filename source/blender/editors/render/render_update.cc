                             ...
                             const bool updated)
{
  ...
  LISTBASE_FOREACH (ARegion *, region, &area->regionbase) {
     ...
     * time of the last update) */
    {
      ...
      drw_context.area = area;
      drw_context.region = region;
      drw_context.v3d = v3d;
      drw_context.engine_type = engine_type;
      drw_context.window = window;
      DRW_notify_view_update(&drw_context, updated);
    }
  }
}
...
void ED_render_engine_changed(Main *bmain, const bool update_scene_data)
{
  ...
  LISTBASE_FOREACH (CacheFile *, cachefile, &bmain->cachefiles) {
     ...
     * We do not use #BKE_cachefile_uses_render_procedural here as we need to update regardless
     * of the current engine or its settings. */
