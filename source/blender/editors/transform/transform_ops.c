  ...
  for (int i = 0; i < kmi_modals_len; i++) {
    ...
    if (kmi) {
      ...
      do {
        ...
        if (!kmi_new->shift && shift_tries < 3) {
          kmi_new->shift = true;
          shift_tries++;
          continue;
        }
        else if (shift_tries == 1) {
          kmi_new->shift = false;
          shift_tries++;
        }

        if (!kmi_new->ctrl && ctrl_tries < 3) {
          kmi_new->ctrl = true;
          ctrl_tries++;
          continue;
        }
        else if (ctrl_tries == 1) {
          kmi_new->ctrl = false;
          ctrl_tries++;
        }

        if (!kmi_new->oskey && oskey_tries < 3) {
          kmi_new->oskey = true;
          oskey_tries++;
          continue;
        }
        else if (oskey_tries == 1) {
          kmi_new->oskey = false;
          oskey_tries++;
        }
        else {
