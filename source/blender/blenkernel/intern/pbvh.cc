...
static void pbvh_vertex_color_get(PBVH *pbvh, int vertex, float r_color[4])
{
  if (pbvh->vcol_domain == ATTR_DOMAIN_CORNER) {
    ...
    for (int i : IndexRange(melem->count)) {
      ...
      Helper::ColType *col = static_cast<Helper::ColType *>(pbvh->vcol->data) + mp->loopstart;
    ...
    Helper::ColType *col = static_cast<Helper::ColType *>(pbvh->vcol->data) + vertex;
...
{
  Helper::ColType *col = reinterpret_cast<Helper::ColType *>(color_attr);
