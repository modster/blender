  ...
  void call(IndexMask mask, fn::MFParams params, fn::MFContext UNUSED(context)) const override
  {
    ...
    if (!sampled_tangents.is_empty()) {
      for (const int i : mask) {
        ...
        sampled_tangents[i] = float3::normalize(sample_with_lookup(lookup, evaluated_tangents));
      }
    }
    ...
    if (!sampled_normals.is_empty()) {
      for (const int i : mask) {
        ...
        sampled_normals[i] = float3::normalize(sample_with_lookup(lookup, evaluated_normals));
      }
    }
  }
};
