                               ...
                               const Field<float3> &offset_field)
{
  ...
  const int new_vert_size = new_vert_orig_indices.size();
  const int connect_edge_offset = orig_edge_size;
  const int connect_edge_size = new_vert_size;
  const int duplicate_edge_offset = connect_edge_offset + connect_edge_size;
  const int duplicate_edge_size = edge_selection.size();
  const int new_edge_size = connect_edge_size + duplicate_edge_size;
  /* Each edge selected for extrusion is extruded into a single vertex. */
  const int new_poly_size = edge_selection.size();
  const int new_loop_size = new_poly_size * 4;

  expand_mesh_size(mesh, new_vert_size, new_edge_size, new_poly_size, new_loop_size);

  MutableSpan<MVert> verts{mesh.mvert, mesh.totvert};
  MutableSpan<MVert> new_verts = verts.take_back(new_vert_size);
  MutableSpan<MEdge> edges{mesh.medge, mesh.totedge};
  MutableSpan<MEdge> connect_edges = edges.slice(connect_edge_offset, connect_edge_size);
  MutableSpan<MEdge> duplicate_edges = edges.slice(duplicate_edge_offset, duplicate_edge_size);
  MutableSpan<MPoly> polys{mesh.mpoly, mesh.totpoly};
  ...
  for (const int i : new_polys.index_range()) {
    MPoly &poly = new_polys[i];
    poly.loopstart = orig_loops.size() + i * 4;
    poly.totloop = 4;
    poly.mat_nr = 0;
    poly.flag = 0;
  }
   ...
   * indices into the `extrude_edges` array, and the element index corresponds to the vert in
   * `new_verts` of the same index. */
  Array<int> new_vert_to_connect_edge(new_vert_size);
  for (const int i : connect_edges.index_range()) {
  ...
  for (const int i : edge_selection.index_range()) {
    ...
    poly_loops[0].e = orig_edge_index;
    poly_loops[1].v = orig_edge.v2;
    poly_loops[1].e = connect_edge_offset + connect_edge_index_2;
    ...
    poly_loops[3].e = connect_edge_offset + connect_edge_index_1;
  }
  ...
  for (const int i_poly : poly_selection) {
    ...
    for (MLoop &loop : loops.slice(poly.loopstart, poly.totloop)) {
      ...
      if (duplicate_edge_indices[loop.e] != -1) {
        loop.e = duplicate_edge_offset + duplicate_edge_indices[loop.e];
      }
    }
  }
  ...
  component.attribute_foreach([&](const AttributeIDRef &id, const AttributeMetaData meta_data) {
    if (meta_data.domain == ATTR_DOMAIN_POINT) {
      ...
      attribute_math::convert_to_static_type(meta_data.data_type, [&](auto dummy) {
        using T = decltype(dummy);
        MutableSpan<T> data = attribute.as_span().typed<T>();
        MutableSpan<T> new_data = data.slice(orig_vert_size, new_vert_size);
