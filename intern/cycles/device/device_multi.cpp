...
class MultiDevice : public Device {
 public:
  ...
  bool apply_delta_compression(device_memory &orig_mem,
                               device_memory &compressed_mem,
                               size_t offset,
                               size_t size,
                               float min_delta,
                               float max_delta) override
  {
    foreach (SubDevice &sub, devices) {
      ...
      if (!sub.device->apply_delta_compression(
              orig_mem, compressed_mem, offset, size, min_delta, max_delta)) {
        return false;
      }
    }
